import 'package:doom_core/src/video/frame_buffer.dart';

abstract final class _TextConstants {
  static const int charWidth = 8;
  static const int charHeight = 8;
  static const int firstChar = 32;
  static const int lastChar = 127;
}

class TextRenderer {
  static const int charWidth = _TextConstants.charWidth;
  static const int charHeight = _TextConstants.charHeight;

  static void drawChar(
    FrameBuffer frame,
    int x,
    int y,
    int charCode,
    int colorIndex,
  ) {
    if (charCode < _TextConstants.firstChar || charCode > _TextConstants.lastChar) {
      return;
    }

    final glyphIndex = charCode - _TextConstants.firstChar;
    if (glyphIndex >= _font.length) return;

    final glyph = _font[glyphIndex];
    for (var row = 0; row < _TextConstants.charHeight; row++) {
      final rowBits = glyph[row];
      for (var col = 0; col < _TextConstants.charWidth; col++) {
        if ((rowBits & (0x80 >> col)) != 0) {
          frame.setPixel(x + col, y + row, colorIndex);
        }
      }
    }
  }

  static void drawString(
    FrameBuffer frame,
    int x,
    int y,
    String text,
    int colorIndex,
  ) {
    var cursorX = x;
    for (var i = 0; i < text.length; i++) {
      final charCode = text.codeUnitAt(i);
      drawChar(frame, cursorX, y, charCode, colorIndex);
      cursorX += _TextConstants.charWidth;
    }
  }

  static void drawStringCentered(
    FrameBuffer frame,
    int y,
    String text,
    int colorIndex,
  ) {
    final width = text.length * _TextConstants.charWidth;
    final x = (FrameBuffer.width - width) ~/ 2;
    drawString(frame, x, y, text, colorIndex);
  }

  static int stringWidth(String text) => text.length * _TextConstants.charWidth;

  static const List<List<int>> _font = [
    [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00], // 32 (space)
    [0x18, 0x18, 0x18, 0x18, 0x18, 0x00, 0x18, 0x00], // 33 !
    [0x6C, 0x6C, 0x24, 0x00, 0x00, 0x00, 0x00, 0x00], // 34 "
    [0x6C, 0x6C, 0xFE, 0x6C, 0xFE, 0x6C, 0x6C, 0x00], // 35 #
    [0x18, 0x3E, 0x60, 0x3C, 0x06, 0x7C, 0x18, 0x00], // 36 $
    [0x00, 0x66, 0xAC, 0xD8, 0x36, 0x6A, 0xCC, 0x00], // 37 %
    [0x38, 0x6C, 0x68, 0x76, 0xDC, 0xCE, 0x7B, 0x00], // 38 &
    [0x18, 0x18, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00], // 39 '
    [0x0C, 0x18, 0x30, 0x30, 0x30, 0x18, 0x0C, 0x00], // 40 (
    [0x30, 0x18, 0x0C, 0x0C, 0x0C, 0x18, 0x30, 0x00], // 41 )
    [0x00, 0x66, 0x3C, 0xFF, 0x3C, 0x66, 0x00, 0x00], // 42 *
    [0x00, 0x18, 0x18, 0x7E, 0x18, 0x18, 0x00, 0x00], // 43 +
    [0x00, 0x00, 0x00, 0x00, 0x00, 0x18, 0x18, 0x30], // 44 ,
    [0x00, 0x00, 0x00, 0x7E, 0x00, 0x00, 0x00, 0x00], // 45 -
    [0x00, 0x00, 0x00, 0x00, 0x00, 0x18, 0x18, 0x00], // 46 .
    [0x06, 0x0C, 0x18, 0x30, 0x60, 0xC0, 0x80, 0x00], // 47 /
    [0x7C, 0xCE, 0xDE, 0xF6, 0xE6, 0xC6, 0x7C, 0x00], // 48 0
    [0x18, 0x38, 0x18, 0x18, 0x18, 0x18, 0x7E, 0x00], // 49 1
    [0x3C, 0x66, 0x06, 0x1C, 0x30, 0x66, 0x7E, 0x00], // 50 2
    [0x3C, 0x66, 0x06, 0x1C, 0x06, 0x66, 0x3C, 0x00], // 51 3
    [0x1C, 0x3C, 0x6C, 0xCC, 0xFE, 0x0C, 0x1E, 0x00], // 52 4
    [0x7E, 0x62, 0x60, 0x7C, 0x06, 0x66, 0x3C, 0x00], // 53 5
    [0x1C, 0x30, 0x60, 0x7C, 0x66, 0x66, 0x3C, 0x00], // 54 6
    [0x7E, 0x66, 0x06, 0x0C, 0x18, 0x18, 0x18, 0x00], // 55 7
    [0x3C, 0x66, 0x66, 0x3C, 0x66, 0x66, 0x3C, 0x00], // 56 8
    [0x3C, 0x66, 0x66, 0x3E, 0x06, 0x0C, 0x38, 0x00], // 57 9
    [0x00, 0x18, 0x18, 0x00, 0x00, 0x18, 0x18, 0x00], // 58 :
    [0x00, 0x18, 0x18, 0x00, 0x00, 0x18, 0x18, 0x30], // 59 ;
    [0x0C, 0x18, 0x30, 0x60, 0x30, 0x18, 0x0C, 0x00], // 60 <
    [0x00, 0x00, 0x7E, 0x00, 0x7E, 0x00, 0x00, 0x00], // 61 =
    [0x30, 0x18, 0x0C, 0x06, 0x0C, 0x18, 0x30, 0x00], // 62 >
    [0x3C, 0x66, 0x0C, 0x18, 0x18, 0x00, 0x18, 0x00], // 63 ?
    [0x7C, 0xC6, 0xDE, 0xDE, 0xDC, 0xC0, 0x7C, 0x00], // 64 @
    [0x18, 0x3C, 0x66, 0x66, 0x7E, 0x66, 0x66, 0x00], // 65 A
    [0xFC, 0x66, 0x66, 0x7C, 0x66, 0x66, 0xFC, 0x00], // 66 B
    [0x3C, 0x66, 0xC0, 0xC0, 0xC0, 0x66, 0x3C, 0x00], // 67 C
    [0xF8, 0x6C, 0x66, 0x66, 0x66, 0x6C, 0xF8, 0x00], // 68 D
    [0xFE, 0x62, 0x68, 0x78, 0x68, 0x62, 0xFE, 0x00], // 69 E
    [0xFE, 0x62, 0x68, 0x78, 0x68, 0x60, 0xF0, 0x00], // 70 F
    [0x3C, 0x66, 0xC0, 0xC0, 0xCE, 0x66, 0x3E, 0x00], // 71 G
    [0x66, 0x66, 0x66, 0x7E, 0x66, 0x66, 0x66, 0x00], // 72 H
    [0x7E, 0x18, 0x18, 0x18, 0x18, 0x18, 0x7E, 0x00], // 73 I
    [0x1E, 0x0C, 0x0C, 0x0C, 0xCC, 0xCC, 0x78, 0x00], // 74 J
    [0xE6, 0x66, 0x6C, 0x78, 0x6C, 0x66, 0xE6, 0x00], // 75 K
    [0xF0, 0x60, 0x60, 0x60, 0x62, 0x66, 0xFE, 0x00], // 76 L
    [0xC6, 0xEE, 0xFE, 0xD6, 0xC6, 0xC6, 0xC6, 0x00], // 77 M
    [0xC6, 0xE6, 0xF6, 0xDE, 0xCE, 0xC6, 0xC6, 0x00], // 78 N
    [0x38, 0x6C, 0xC6, 0xC6, 0xC6, 0x6C, 0x38, 0x00], // 79 O
    [0xFC, 0x66, 0x66, 0x7C, 0x60, 0x60, 0xF0, 0x00], // 80 P
    [0x38, 0x6C, 0xC6, 0xC6, 0xDA, 0x6C, 0x36, 0x00], // 81 Q
    [0xFC, 0x66, 0x66, 0x7C, 0x6C, 0x66, 0xE6, 0x00], // 82 R
    [0x3C, 0x66, 0x70, 0x38, 0x0E, 0x66, 0x3C, 0x00], // 83 S
    [0x7E, 0x5A, 0x18, 0x18, 0x18, 0x18, 0x3C, 0x00], // 84 T
    [0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x3C, 0x00], // 85 U
    [0x66, 0x66, 0x66, 0x66, 0x66, 0x3C, 0x18, 0x00], // 86 V
    [0xC6, 0xC6, 0xC6, 0xD6, 0xFE, 0xEE, 0xC6, 0x00], // 87 W
    [0xC6, 0xC6, 0x6C, 0x38, 0x6C, 0xC6, 0xC6, 0x00], // 88 X
    [0x66, 0x66, 0x66, 0x3C, 0x18, 0x18, 0x3C, 0x00], // 89 Y
    [0xFE, 0xC6, 0x8C, 0x18, 0x32, 0x66, 0xFE, 0x00], // 90 Z
    [0x3C, 0x30, 0x30, 0x30, 0x30, 0x30, 0x3C, 0x00], // 91 [
    [0xC0, 0x60, 0x30, 0x18, 0x0C, 0x06, 0x02, 0x00], // 92 backslash
    [0x3C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x3C, 0x00], // 93 ]
    [0x10, 0x38, 0x6C, 0xC6, 0x00, 0x00, 0x00, 0x00], // 94 ^
    [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFE], // 95 _
    [0x18, 0x18, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00], // 96 `
    [0x00, 0x00, 0x3C, 0x06, 0x3E, 0x66, 0x3B, 0x00], // 97 a
    [0xE0, 0x60, 0x7C, 0x66, 0x66, 0x66, 0xDC, 0x00], // 98 b
    [0x00, 0x00, 0x3C, 0x66, 0x60, 0x66, 0x3C, 0x00], // 99 c
    [0x0E, 0x06, 0x3E, 0x66, 0x66, 0x66, 0x3B, 0x00], // 100 d
    [0x00, 0x00, 0x3C, 0x66, 0x7E, 0x60, 0x3C, 0x00], // 101 e
    [0x1C, 0x36, 0x30, 0x78, 0x30, 0x30, 0x78, 0x00], // 102 f
    [0x00, 0x00, 0x3B, 0x66, 0x66, 0x3E, 0x06, 0x7C], // 103 g
    [0xE0, 0x60, 0x6C, 0x76, 0x66, 0x66, 0xE6, 0x00], // 104 h
    [0x18, 0x00, 0x38, 0x18, 0x18, 0x18, 0x3C, 0x00], // 105 i
    [0x06, 0x00, 0x0E, 0x06, 0x06, 0x66, 0x66, 0x3C], // 106 j
    [0xE0, 0x60, 0x66, 0x6C, 0x78, 0x6C, 0xE6, 0x00], // 107 k
    [0x38, 0x18, 0x18, 0x18, 0x18, 0x18, 0x3C, 0x00], // 108 l
    [0x00, 0x00, 0x6C, 0xFE, 0xD6, 0xD6, 0xC6, 0x00], // 109 m
    [0x00, 0x00, 0xDC, 0x66, 0x66, 0x66, 0x66, 0x00], // 110 n
    [0x00, 0x00, 0x3C, 0x66, 0x66, 0x66, 0x3C, 0x00], // 111 o
    [0x00, 0x00, 0xDC, 0x66, 0x66, 0x7C, 0x60, 0xF0], // 112 p
    [0x00, 0x00, 0x3B, 0x66, 0x66, 0x3E, 0x06, 0x0F], // 113 q
    [0x00, 0x00, 0xDC, 0x76, 0x60, 0x60, 0xF0, 0x00], // 114 r
    [0x00, 0x00, 0x3C, 0x60, 0x3C, 0x06, 0x7C, 0x00], // 115 s
    [0x30, 0x30, 0x7C, 0x30, 0x30, 0x36, 0x1C, 0x00], // 116 t
    [0x00, 0x00, 0x66, 0x66, 0x66, 0x66, 0x3B, 0x00], // 117 u
    [0x00, 0x00, 0x66, 0x66, 0x66, 0x3C, 0x18, 0x00], // 118 v
    [0x00, 0x00, 0xC6, 0xD6, 0xD6, 0xFE, 0x6C, 0x00], // 119 w
    [0x00, 0x00, 0xC6, 0x6C, 0x38, 0x6C, 0xC6, 0x00], // 120 x
    [0x00, 0x00, 0x66, 0x66, 0x66, 0x3E, 0x06, 0x7C], // 121 y
    [0x00, 0x00, 0x7E, 0x4C, 0x18, 0x32, 0x7E, 0x00], // 122 z
    [0x0E, 0x18, 0x18, 0x70, 0x18, 0x18, 0x0E, 0x00], // 123 {
    [0x18, 0x18, 0x18, 0x00, 0x18, 0x18, 0x18, 0x00], // 124 |
    [0x70, 0x18, 0x18, 0x0E, 0x18, 0x18, 0x70, 0x00], // 125 }
    [0x3B, 0x6E, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00], // 126 ~
    [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00], // 127 DEL
  ];
}
